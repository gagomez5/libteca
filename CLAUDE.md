...
- **Backend**: Supabase (Postgres + Auth), configured via `SUPABASE_URL` / `SUPABASE_ANON_KEY` constants near the top of `js/db.js`. Tables used: `books`, `wishlist`, `profile`, `notifications`, `notification_receipts`.
